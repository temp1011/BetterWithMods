...
public class SteelShapedRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting matrix, World world) {
        for (int i = 0; i <= SteelCraftingManager.WIDTH - this.recipeWidth; ++i) {
            for (int j = 0; j <= SteelCraftingManager.HEIGHT - this.recipeHeight; ++j) {
    ...
    private boolean checkMatch(InventoryCrafting matrix, int x, int y, boolean mirrored) {
        for (int k = 0; k < SteelCraftingManager.WIDTH; ++k) {
            for (int l = 0; l < SteelCraftingManager.HEIGHT; ++l) {
                int i1 = k - x;
                int j1 = l - y;
